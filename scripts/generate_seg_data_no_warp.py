...
data_path = os.path.join(root_path, 'dataset/seg_teeth_eq_v2')
...
def main():
    ...
    # for i in range(5):
        ...
        for b in range(boxes.shape[0]):
            ...
            im_out = np.zeros((OUT_IM_SIZE[0],
                               OUT_IM_SIZE[1], 3)).astype(np.uint8)
            width = cropped_im.shape[1]
            length = cropped_im.shape[0]
            new_x, new_y = math.floor((OUT_IM_SIZE[1] - width) / 2), \
                           math.floor((OUT_IM_SIZE[0] - length) / 2)
            end_x, end_y = new_x + width, new_y + length
            im_out[new_y:end_y, new_x:end_x] = cropped_im
            # print(im_out.shape)

            # Save the bounding box as the label image
            la_out = np.zeros((OUT_IM_SIZE[0],
                               OUT_IM_SIZE[1])).astype(np.uint8)
            width = box_range[2]
            length = box_range[3]
            new_x, new_y = math.floor((OUT_IM_SIZE[1] - width) / 2), \
                           math.floor((OUT_IM_SIZE[0] - length) / 2)
